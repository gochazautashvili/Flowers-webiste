...
function FlowersFilter() {
  ...
  document.addEventListener("click", (e) => {
    if (
      e.target.className !== "flowersFilter__left_select" &&
  ...
  const handleSearching = (searchTerm) => {
    setLoading(true);
    axios
      .get(`${API}/product/search?searchTerm=${searchTerm}`, {
        headers: { Authorization: `Bearer ${user?.token}` },
      })
      .then((res) => {
        setFlowers(res.data);
  ...
  const handleCategory = (searchByCategory) => {
    ...
    axios
      .get(
        `${API}/product/searchByCategory?searchByCategory=${searchByCategory}`,
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        }
      )
  ...
  return (
    <section className="flowersFilter">
        ...
        <div className="flowersFilter__inner">
          <div className="flowersFilter__left">
            <div className="flowersFilter__left_box">
              <div
                className="flowersFilter__left_select"
                onClick={() => {
                  setShowCategory(!showCategory);
                }}
              >
                {categoryValue}{" "}
                <img loading="lazy" className="i" src={Dropdown__icon} alt="" />
              </div>
              {showCategory && (
                <div className="flowersFilter__left_box_content">
          ...
          <div className="flowersFilter__right">
            ...
            <label htmlFor="search" onClick={handleSearch}>
              <img loading="lazy" className="search__icon" src={Search__icon} alt="" />
            </label>
          </div>
        </div>
      </div>
      ...
      <div className="flowersFilter__showMore_box">
        <button onClick={handleShowPrev} className="flowersFilter__showMore">
          <img loading="lazy" src={Prev__icon} alt="" /> უკან
        </button>
        <button onClick={handleShowNext} className="flowersFilter__showMore">
          წინ <img loading="lazy" src={Next__icon} alt="" />
        </button>
      </div>
    </section>
  );
