...
function SingleProduct() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${API}/product/single_product?productID=${id}`, {
  ...
  const addWishlist = (flowerProp) => {
    ...
    axios
      .post(
        `${API}/wishlist`,
        { flowerProp, cartUserID: user.userID, productID: flower._id },
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        }
      )
        ...
        {loading ? (
          ...
          <div className="single__inner">
            <div className="single__left">
              <Image mainImg={flower?.mainImg} title={flower?.title} width={600} height={700} />
            </div>
              ...
              {user && (
                <>
                  ...
                  <div className="single__right_bottom">
                    <button
                      onClick={() => addWishlist(flower)}
                      className="wishlist"
                    >
                      {loadingWish ? (
                        <CreateLoader />
                      ) : wishlistDisable.length > 0 ? (
                        "Remove Wishlist"
                      ) : (
                        "Add WishList"
                      )}
                    ...
                    <button
                      ...
                      className="add_cart"
                    >
                      {loadingCart ? <CreateLoader /> : "ADD TO CART"}
                    </button>
                  </div>
                </>
              )}
